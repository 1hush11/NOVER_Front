<template>
  <div class="flex items-center justify-between rounded-lg shadow p-4 w-full">
    <div class="flex items-center gap-4" @click="goToTrackPage">
    ...
    <div class="flex items-center gap-4">
      ...
      <button
        ...
        @click.stop="handleAddToLibraryClick"
      >
        <svg
          fill="#1c1c1"
          width="25px"
...
async function addToLibrary() {
  try {
    const res = await fetch(
      `http://localhost:5240/api/user/library/add_track/${props.track.id}`,
      {
        method: 'POST',
        credentials: 'include'
      }
    ...
    toast.error(err.message || 'Ошибка добавления трека в медиатеку', {
      autoClose: 3000,
      position: 'bottom-center'
    })
  }
