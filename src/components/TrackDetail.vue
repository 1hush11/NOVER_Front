  ...
  <div class="flex-1 p-4">
    ...
    <img v-if="track.cover" :src="track.cover" alt="track cover" class="cover-image m-6" />
      <div class="flex-1">
        ...
        <input type="range" min="0" max="100" v-model="progress" class="track-slider" />
...
onMounted(async () => {
  try {
    ...
    if (!trackId) {
      console.error('ID трека не найден.');
      return;
    }

    const response = await fetch(`http://localhost:5240/api/track/tracks/${trackId}`, {
      credentials: 'include',
    });

    if (!response.ok) {
      console.error('Ошибка загрузки трека:', await response.text());
      return;
    }
    ...
    track.value = {
      id: data.id,
      title: data.name,
      singer: data.singers.length ? data.singers.join(', ') : 'Неизвестный исполнитель',
      genre: data.genreName,
      cover: '/src/resources/trackCovers/' + data.coverUrl || '/src/icons/NOVER_icon.ico',
    ...
    const similarResponse = await fetch(`http://localhost:5240/api/track/similar/${trackId}?count=5`, {
      credentials: 'include'
    });

    if (!similarResponse.ok) {
      console.error('Ошибка загрузки похожих треков:', await similarResponse.text());
      return;
    }

    const similarData = await similarResponse.json();

    similarTracks.value = similarData.map(track => ({
      id: track.id,
      title: track.name,
    ...
    console.error('Ошибка при загрузке трека и похожих треков:', error);
  }
});
